...
class NarrativeGenerationModule:
    ...
    def _generate_analyst_insights(self, df: pd.DataFrame, kpis: Dict, growth_rates: Dict) -> List[str]:
        ...
        if 'Marketing Spend' in df.columns and 'revenue' in df.columns:
            ...
            if roas < 2:
                insights.append(
                    f"Marketing ROI is concerning — ROAS is {roas:.2f}. "
        ...
        if 'liabilities' in df.columns and 'date' in df.columns:
            ...
            if liability_trend > 2:
                insights.append(
                    f"Liabilities growing at {liability_trend:.1f}% monthly average — leverage risk rising. "
        ...
        if 'ar' in df.columns and 'revenue' in df.columns and 'date' in df.columns:
            ...
            if len(df_sorted) >= 6:
                ...
                if recent_ar_growth > recent_rev_growth + 5:
                    insights.append(
                        f"AR increasing faster than revenue ({recent_ar_growth:.1f}% vs {recent_rev_growth:.1f}%) "
        ...
        if dso > 45:
            insights.append(
                f"DSO at {dso:.1f} days is elevated. Cash conversion cycle is stretched. "
        ...
        if 'expenses' in df.columns and 'revenue' in df.columns and 'date' in df.columns:
            ...
            if len(df_sorted) >= 12:
                ...
                if recent_expense_growth > recent_revenue_growth:
                    insights.append(
                        f"Expenses growing faster than revenue YoY "
        ...
        if 'ap' in df.columns and 'expenses' in df.columns and 'date' in df.columns:
            ...
            if len(df_sorted) >= 6:
                ...
                if recent_ap_change < -5 and recent_expense_change > 0:
                    insights.append(
                        f"AP decreasing ({recent_ap_change:.1f}%) while expenses rising "
        ...
        if profit_margin < 10:
            insights.append(
                f"Profit margin at {profit_margin:.1f}% is below optimal levels. "
        ...
        if rev_growth > 20:
            ...
            insights.append(
                f"Revenue declining {abs(rev_growth):.1f}% QoQ. "
        ...
        if 'equity' in df.columns and 'date' in df.columns:
            ...
            if len(df_sorted) >= 6:
                ...
                if equity_trend < -2:
                    insights.append(
                        f"Equity eroding at {abs(equity_trend):.1f}% monthly average. "
        ...
        if 'cash_conversion_cycle' in df.columns:
            ...
            if ccc > 60:
                insights.append(
                    f"Cash conversion cycle at {ccc:.0f} days is extended. "
