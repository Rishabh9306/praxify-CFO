...
### Base URL
```
Production:  https://api.praxifi.io/api
...
docker build -t praxifi-cfo:latest .
...
# Expose service
kubectl expose deployment praxifi-cfo --type=LoadBalancer --port=8000

# Scale deployment
kubectl scale deployment praxifi-cfo --replicas=5
```
...
```bash
# Initialize EB CLI
eb init -p python-3.9 praxifi-cfo

# Create environment
eb create praxifi-production
...
gcloud builds submit --tag gcr.io/PROJECT_ID/praxifi-cfo

# Deploy to Cloud Run
gcloud run deploy praxifi-cfo \
  --image gcr.io/PROJECT_ID/praxifi-cfo \
  --platform managed \
  --memory 8Gi \
  --cpu 4 \
  --max-instances 10
...
az group create --name praxifi-rg --location eastus

# Deploy container
az container create \
  --resource-group praxifi-rg \
  --name praxifi-cfo \
  --image praxifi-cfo:latest \
  --cpu 4 \
  --memory 8 \
...
1. **Fork the repository**
   ```bash
   git clone https://github.com/YOUR_USERNAME/praxifi-CFO.git
   cd praxifi-CFO
...
| 🐛 **Issues** | [GitHub Issues](https://github.com/Rishabh9306/praxifi-CFO/issues) |
| 💬 **Discussions** | [GitHub Discussions](https://github.com/Rishabh9306/praxifi-CFO/discussions) |
...
@software{praxifi_cfo_2025,
  ...
  author = {Praxifi Team},
  year = {2025},
  url = {https://github.com/Rishabh9306/praxifi-CFO},
  version = {1.0}
}
```
