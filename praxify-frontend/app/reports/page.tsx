...
export default function ReportsPage() {
  ...
  const handleExportJSON = (sessionId: string) => {
    ...
    link.href = url;
    link.download = `praxify-cfo-session-${sessionId}.json`;
    link.click();
  ...
  const handleExportConversation = (sessionId: string) => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `praxify-cfo-conversation-${sessionId}.txt`;
    link.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleDeleteSession = (sessionId: string) => {
    if (confirm('Are you sure you want to delete this session? This action cannot be undone.')) {
      ...
      localStorage.setItem('praxify-cfo-sessions', JSON.stringify(updatedHistory));
